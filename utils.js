...
function updateEnv(variable, value) {
  ...
  fs.readFile(envFilePath, "utf8", (err, data) => {
    ...
    // Check if the variable doesn't exist in the file, add it at the end
    if (!regex.test(data)) {
      newData += `\n${variable}=${value}`;
    }
    ...
    fs.writeFile(envFilePath, newData, "utf8", (err) => {
      if (err) {
        console.error("Unable to write to .env file:", err);
      } else {
...
function isExpiredToken(token) {
  ...
  try {
    ...
    if (parsedPayload.exp) {
      ...
      this.log(colors.cyan(`Has the token expired? ${isExpired ? "Yes, you need to replace the token" : "Not yet, you can continue"}`));
      return isExpired;
    } else {
      this.log(colors.yellow(`The token is permanent, expiration date cannot be read`));
      return false;
    }
  } catch (error) {
